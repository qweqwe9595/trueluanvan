...
function MovieDetailReview({ movie }) {
  ...
  const sendReview = async (e) => {
    ...
    setReviews((prev) => [res.data.reviewsQuery, ...prev]);
  ...
  return (
    <div className="w-full flex flex-col gap-4 mt-2 font-dosis">
      ...
      {rating && <RatingModal openProp={[rating, setRating]} movie={movie} />}
      {reviewSuccess && (
        <ErrorLogin
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 ">
            <img
              ...
              alt=""
            />
            <span className="font-bold text-xl capitalize">{user?.email}</span>
          </div>
          <div
